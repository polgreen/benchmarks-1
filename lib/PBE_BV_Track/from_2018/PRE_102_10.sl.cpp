...
#include <stdlib.h>  
#define WIDTH 32
unsigned long int f(unsigned long int x$0) { return (1 & ~x$0 >> 4) == 1 ? ((1 & x$0 >> 16) == 1 ? ((1 & (x$0 >> 1) >> 1) == 1 ? ((1 & x$0) == 1 ? ~(x$0 >> 1) : (x$0 | x$0 << 1) << 1) : (x$0 | x$0 << 1) << 1) : ((1 & (x$0 << 1) >> 16) == 1 ? ~(x$0 >> 1) : (x$0 | x$0 << 1) << 1)) : ((1 & x$0) == 1 ? ~(x$0 >> 1) : (x$0 | x$0 << 1) << 1); }
